<template>
  <div>
    <div class="flex justify-between mb-2">
      ...
      <button @click="openCreateModal" type="button" class="bg-green-800 text-end rounded p-2 text-white">
        Thêm cây mới
      </button>
    </div>
    ...
    <table class="table-auto w-full border-collapse border border-lime-500">
      <thead>
        <tr class="bg-lime-500">
          ...
          <th class="border border-gray-300 px-4 py-2">Tên Khoa Học</th>
          ...
          <th class="border border-gray-300 px-4 py-2">Tưới Nước (lần/tuần)</th>
      ...
      <tbody>
        <tr v-for="plant in paginatedPlants" :key="plant.id">
          ...
          <td class="border border-gray-300 px-4 py-2">{{ plant.productCode }}</td>
          ...
          <td class="border border-gray-300 px-4 py-2">{{ plant.scientificName || 'N/A' }}</td>
          ...
          <td class="border border-gray-300 px-4 py-2">{{ plant.difficulty || 'N/A' }}</td>
          <td class="border border-gray-300 px-4 py-2 text-center">
            {{ plant.wateringMinTimes && plant.wateringMaxTimes ? `${plant.wateringMinTimes} - ${plant.wateringMaxTimes}` : 'N/A' }}
          </td>
...
import { fetchPlants, deletePlant } from '@/stores/plantService'; // Sử dụng hàm từ plantService
...
const loadPlants = async () => {
  ...
  if (fetchedPlants) {
    plants.value = fetchedPlants.sort((a, b) => a.id - b.id);
  } else {
    plants.value = [];
  }
};
...
const handleDeletePlant = async (plantId) => {
  if (confirm(`Bạn có chắc chắn muốn xóa cây với ID ${plantId}?`)) {
