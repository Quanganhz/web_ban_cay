  ...
  <section class="relative py-32 pt-40">
    ...
    <div class="relative z-0 text-left mx-36">
      ...
      <div class="container mx-auto mt-80">
        ...
        <div class="relative h-96 mt-96 flex items-center icon">
          <div class="absolute inset-0 bg-white opacity-5 rounded-3xl"></div>
        ...
        <div class="relative h-96 mt-64 flex items-center icon">
          <div class="absolute inset-0 bg-white opacity-5 rounded-3xl"></div>
...
export default {
  ...
  methods: {
    handleScroll() {
      ...
      icons.forEach((icon) => {
        const move = Math.min(scrollTop / 0, 10); // Giới hạn vị trí di chuyển
        icon.style.transform = `translateX(${move}px)`; // Di chuyển icon từ phải qua trái
        icon.style.transition = 'transform 1s ease-out'; // Thay đổi thời gian để làm mượt
      });
    },
  },
  mounted() {
