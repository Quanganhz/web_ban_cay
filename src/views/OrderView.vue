  ...
  <div class="flex h-screen p-4 space-x-4">
      ...
      <div class="flex space-x-4">
        ...
        <div class="bg-gray-100 p-4 rounded shadow-md flex-grow max-w-[900px]">
          ...
          <div class="flex flex-wrap space-x-2 mb-4">
            <span
              v-for="(value, index) in displayedMainDishes"
              :key="index"
              class="px-2 py-1 border-2 border-red-500 rounded-full"
            >
              {{ value }}
            </span>
            <span
              v-for="(value, index) in displayedVegetables"
              :key="index"
              class="px-2 py-1 border-2 border-yellow-500 rounded-full"
            >
              {{ value }}
            </span>
          </div>
      ...
      <div class="bg-gray-100 p-4 rounded shadow-md">
        ...
        <div class="grid grid-cols-8 gap-1">
          <button
            ...
            class="px-2 py-2 text-black rounded-md hover:bg-red-600"
          >
            {{ button }}
          </button>
          ...
          <button
            ...
            class="py-2 text-black rounded-md hover:bg-yellow-600"
          >
            {{ button }}
          </button>
        </div>
    ...
    <div class="flex flex-col w-1/3 bg-gray-100 p-4 rounded shadow-md overflow-auto">
      ...
      <div v-for="(result, index) in results" :key="index" class="mb-4">
        ...
        <p>
          <strong>Món:</strong> {{ result.mainDishes.join(', ') }},
          {{ result.vegetables.join(', ') }}
        </p>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      buttonsVegetable: ['Rau củ luộc', 'Mướp nhồi thị', 'Rau muống xào', 'Lạc rang', 'Cà pháo'], // Danh sách button rau củ
